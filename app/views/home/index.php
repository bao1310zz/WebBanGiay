        ...
        <?php else: ?>
            ...
            ?>
                <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                    ...
                    <a href="/WebBanGiay/Home/detail/<?= $product->id ?? 0 ?>" class="text-decoration-none">
                                ...
                                <div class="row h-100 align-items-center">
                                    ...
                                    <div class="col-md-6 text-white text-md-left text-center pl-md-5">
                                        <span class="badge badge-warning text-dark text-uppercase font-weight-bold px-3 py-2 mb-3" style="letter-spacing: 1px; font-size: 0.75rem; border-radius: 4px;">
                                            <?= htmlspecialchars($product->category_name ?? 'GIÀY CAO CẤP') ?>
                                        </span>
                ...
                <div class="card product-card h-100">
                    <a href="/WebBanGiay/Home/detail/<?= $product->id ?? 0 ?>" class="text-decoration-none">
                        ...
                        <div class="card-body text-center pb-2">
                            <p class="small text-uppercase mb-2" style="color: #c89b3c; font-weight: 600; letter-spacing: 1px;">
                                <?= htmlspecialchars($product->category_name ?? 'GIÀY CAO CẤP') ?>
                            </p>
                            <h6 class="font-weight-bold text-dark text-uppercase mb-3 text-truncate">
                                <?= htmlspecialchars($product->name ?? 'Tên Sản Phẩm') ?>
                            </h6>
                            <h5 class="font-weight-bold" style="color: #111;">
                    ...
                    <div class="card-footer bg-white border-0 p-3 pt-0">
                        ...
                        <form action="/WebBanGiay/Product/addToCart/<?= $product->id ?? 0 ?>" method="POST" class="ajax-cart-form">
                            <input type="hidden" name="is_ajax" value="1">
                            
                            <div class="d-flex justify-content-between align-items-center mb-2 px-3">
                                <button type="button" class="btn btn-sm btn-outline-dark" style="width: 30px;" onclick="var el = document.getElementById('qty_home_<?= $product->id ?? 0 ?>'); if(el.value > 1) el.value--;">-</button>
                                <input type="number" name="quantity" id="qty_home_<?= $product->id ?? 0 ?>" class="form-control text-center border-0 font-weight-bold" value="1" min="1" readonly style="width: 50px; background: transparent;">
                                <button type="button" class="btn btn-sm btn-outline-dark" style="width: 30px;" onclick="document.getElementById('qty_home_<?= $product->id ?? 0 ?>').value++;">+</button>
                            </div>
                            <button type="submit" class="btn btn-gold btn-block" style="background: #111; color: #fff; font-weight: bold;">
                                <i class="fas fa-shopping-bag mr-2"></i> THÊM VÀO GIỎ
                            </button>
                        </form>

                        <?php if (SessionHelper::isAdmin()): ?>
                            <div class="d-flex justify-content-between align-items-center mt-3 pt-3" style="border-top: 1px dashed #ddd;">
                                <a href="/WebBanGiay/Product/edit/<?= $product->id ?? 0 ?>" class="btn btn-sm btn-outline-dark flex-grow-1 mr-1" style="border-radius: 4px;">
                                    <i class="fas fa-edit text-warning"></i> SỬA
                                </a>
                                <a href="/WebBanGiay/Product/delete/<?= $product->id ?? 0 ?>" class="btn btn-sm btn-outline-danger flex-grow-1 ml-1" style="border-radius: 4px;" onclick="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này?');">
